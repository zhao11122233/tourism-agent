...
class TicketClient:
    ...
    def _mock_query(self, scenic_spot: str, visit_date: str) -> dict:
        """模拟多平台余票查询，返回美团/携程/景区自营三平台数据"""
        base_prices = {
            "故宫": 60, "八达岭长城": 40, "黄山": 190, "西湖": 0,
            "兵马俑": 120, "秦始皇兵马俑博物馆": 120, "颐和园": 30,
            "张家界": 225, "九寨沟": 169, "布达拉宫": 200, "桂林漓江": 215,
        }
        spot_matched = scenic_spot
        for name in base_prices:
            if name in scenic_spot or scenic_spot in name:
                spot_matched = name
                break
        base = base_prices.get(spot_matched, 80)
